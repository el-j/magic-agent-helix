...
export class DockerPlugin implements DetectionPlugin {
	...
	async generateInstructions(
		...
		if (this.hasDockerfile) {
			content += "* This project is containerized using a `Dockerfile`.\n";
			...
			if (dockerfileContent) {
				...
				if (
					...
					dockerfileContent.includes("AS builder")
				) {
					content +=
				...
				const baseImageMatch = dockerfileContent.match(/^FROM\s+([^\s]+)\s*$/m);
