...
export class GitLabCIPlugin implements DetectionPlugin {
	...
	readonly description = "Detects GitLab CI/CD configuration and provides CI/CD-specific instructions";
	...
	detect(context: DetectionContext): DetectionResult {
		...
		if (gitlabCIContent) {
			...
			// Extract stages
			const stagesSection = gitlabCIContent.match(/^stages:\s*\n((?:  - .+\n?)+)/m);
			if (stagesSection) {
				const stages = stagesSection[1].match(/- (.+)/g)?.map(s => s.replace(/- /, '').trim());
				if (stages) {
