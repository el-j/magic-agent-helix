...
export class GitHubActionsPlugin implements DetectionPlugin {
	...
	readonly description = "Detects GitHub Actions workflows and provides CI/CD-specific instructions";
	readonly version = "1.0.0";
	
	detect(context: DetectionContext): DetectionResult {
		// Check for GitHub Actions workflow files
		const hasWorkflows = context.matchesPattern(".github/workflows/*.yml") || 
		...
		if (workflowFiles.length > 0) {
			...
			if (firstWorkflow) {
				...
				metadata.hasMatrixStrategy = firstWorkflow.includes("strategy:") && firstWorkflow.includes("matrix:");
				metadata.hasCaching = firstWorkflow.includes("actions/cache") || firstWorkflow.includes("cache:");
				metadata.hasArtifacts = firstWorkflow.includes("actions/upload-artifact") || 
