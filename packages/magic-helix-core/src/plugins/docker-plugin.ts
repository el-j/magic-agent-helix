...
export class DockerPlugin implements DetectionPlugin {
	...
	readonly description = "Detects Docker usage and provides Docker-specific instructions";
	readonly version = "1.0.0";
	
	detect(context: DetectionContext): DetectionResult {
		// Check for Docker files
		const hasDockerfile = context.hasFile("Dockerfile") || context.matchesPattern("**/Dockerfile");
		const hasDockerCompose = context.hasFile("docker-compose.yml") || 
		...
		if (hasDockerfile) {
			...
			if (dockerfileContent) {
				const stages = (dockerfileContent.match(/FROM\s+\S+\s+AS\s+\S+/gi) || []).length;
				if (stages > 0) {
	...
	generateInstructions(context: DetectionContext, metadata?: Record<string, unknown>): InstructionTemplate[] {
		...
		if (metadata?.hasDockerCompose) {
			instructions.push({
				...
				targetFiles: ["**/docker-compose.yml", "**/docker-compose.yaml", "**/compose.yml", "**/compose.yaml"]
			});
		}
